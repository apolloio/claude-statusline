...
- **Git status markers:** Set CLAUDE_STATUSLINE_GIT_STATUS to dirty or on to show working-tree state next to the branch name — `*` for modified tracked files, `?` for untracked files, and dimmed `↑`/`↓` counts (green/orange) for commits ahead of or behind your upstream. Off by default. CLAUDE_STATUSLINE_GIT_UNTRACKED=off hides the `?` marker.
