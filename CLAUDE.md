...
2. **Persistent state** (all under `$_CLAUDE_DIR`, which resolves from `CLAUDE_STATUSLINE_STATE_DIR` env var, defaulting to `~/.claude/`):
   ...
   - `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/tasks/<session_id>/` — **read-only**, not under `$_CLAUDE_DIR`/`CLAUDE_STATUSLINE_STATE_DIR`. Claude Code's own live task state (one JSON file per task); powers the opt-in todo-progress segment (`CLAUDE_STATUSLINE_TODOS`). This script never writes here.
3. **Line 1** — yellow CWD (shortened to fit terminal; see `CLAUDE_STATUSLINE_CWD_MAXLEN`, default 64), green git branch (shortened, `CLAUDE_STATUSLINE_BRANCH_MAXLEN` default 64; branch gets priority in budget; resolved before CWD so its actual length drives the CWD budget), optional 🧠, magenta model name, effort/fast-mode glyphs, colored context %
4. **Line 2** — perf dots (4-dot cache/latency badge from transcript JSONL), optional todo/task progress (`CLAUDE_STATUSLINE_TODOS`, off by default), optional Pro/Max or Enterprise budget segments, ∑ˢ/∑ⁱ cost badges, 💸 rolling 15m/1h/1d spend windows
